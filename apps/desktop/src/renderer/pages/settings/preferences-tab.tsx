...
export function PreferencesTab(): JSX.Element {
  ...
  const onAiToggle = (next: boolean): void => {
    void patchMe({ aiCategorizationMode: next ? 'ON' : 'OFF' })
  }
    ...
    <Panel>
      <div className="flex max-w-md flex-col gap-5">
        ...
        <Field
          ...
          hint={t('settings.aiSubtitle')}
        >
          <label className="flex cursor-pointer select-none items-center gap-2.5 rounded-sm border border-line bg-surface-2 px-3 py-2 text-wm-sm text-fg">
            <input
              type="checkbox"
              checked={aiMode === 'ON'}
              ...
              className="wm-check"
            />
            <span>{t('settings.aiToggleLabel')}</span>
          </label>
        </Field>
